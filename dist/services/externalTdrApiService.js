...
exports.fetchOfficialExternalData = fetchOfficialExternalData;
exports.normalizeExternalSourceData = normalizeExternalSourceData;
const axios_1 = __importDefault(require("axios"));
function getAxiosClient() {
    const baseURL = process.env.EXTERNAL_TDR_BASE_URL;
    if (!baseURL) {
        const err = new Error('EXTERNAL_TDR_BASE_URL is missing');
        err.statusCode = 500;
        throw err;
    }
...
async function postWithRetry(client, path, body) {
    ...
    const err = new Error(`External API failed: POST ${path}`);
    err.statusCode = 502;
    err.cause = lastError;
    throw err;
}
async function fetchOfficialExternalData(application_id, samagra_id) {
...
function normalizeExternalSourceData(raw) {
...
//# sourceMappingURL=externalTdrApiService.js.map